...
function computeExpected(pick, allPicks, market) {
  const period = 'm1';
  const val = (p) => p[period] ?? 0;

  const sameConf = allPicks.filter(p =>
    p.market === market && p.confidence === pick.confidence && p[period] != null
  );
  const srcA = sameConf.length
    ? sameConf.reduce((s, p) => s + val(p), 0) / sameConf.length
    : 0;

  const sameTicker = allPicks.filter(p => p.ticker === pick.ticker && p[period] != null);
  const srcB = sameTicker.length >= 2
    ? sameTicker.reduce((s, p) => s + val(p), 0) / sameTicker.length
    : srcA;

  const sameSector = allPicks.filter(p =>
    p.market === market && p.sector === pick.sector && p.confidence === pick.confidence && p[period] != null
  );
  const srcC = sameSector.length
    ? sameSector.reduce((s, p) => s + val(p), 0) / sameSector.length
    : srcA;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
          ...
          <div style={{
            fontSize: 11, color: timing.change != null ? (timing.change >= 0 ? C.up : C.down) : C.textSubtle,
            fontVariantNumeric: 'tabular-nums',
          }}>
            {timing.change != null ? `${timing.change >= 0 ? '+' : ''}${timing.change.toFixed(1)}%` : '—'}
          </div>
        </div>
      </div>
