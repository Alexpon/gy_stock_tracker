...
def _seed_db():
    """Seed DB with deterministic data that covers all verification scenarios.

    Two episodes:
      EP654 (2026-04-18) — transcript ✓, 9 picks (5 TW + 4 US), no performance data
      EP630 (2026-01-24) — transcript ✓, 6 picks (4 TW + 2 US), with w1/w2/m1 data

    This gives us:
      - Analysis stats to verify (avg returns, hit rates, sub-counts)
      - Action page filtering (doing/watching shown, mention hidden)
      - Episodes page status (both partial, neither completed)
      - Price formatting (commas, decimals, percentages)
    """
...
def server_url(tmp_path_factory):
    ...
    tmp_dir = tmp_path_factory.mktemp("journeys")
    ...
    # Mock RSS to avoid external network calls during tests
    from unittest.mock import patch
    mock_rss = patch("backend.rss.check_new", return_value=[])
    mock_download = patch("backend.rss.download_audio")
    mock_rss.start()
    mock_download.start()
