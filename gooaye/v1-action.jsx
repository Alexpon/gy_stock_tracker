// V1 Action: 決策頁 — 最新 4 集提到的可跟標的
// 每檔一張大卡，顯示預期 1M 回報（含信心區間）+ 進場時機提示

// ─── 計算引擎 ───────────────────────────────────────
function V1A_computeExpected(pick, allPicks, market) {
  // 三個獨立來源 → 平均 → 取標準差當區間
  const period = 'm1';
...
// ─── 卡片 ───────────────────────────────────────────
function V1A_ActionCard({ pick, episode, allPicks }) {
  const market = pick.market;
  const exp = V1A_computeExpected(pick, allPicks, market);
  const timing = V1A_entryTiming(pick);
  ...
  return (
    <div style={{
      background: V1_C.surface,
      border: `1px solid ${V1_C.border}`,
      borderRadius: 10,
      overflow: 'hidden',
      display: 'flex',
      ...
      <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${V1_C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
              <span style={{
                fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-mono)',
                color: V1_C.text, letterSpacing: '-0.02em',
              }}>{pick.ticker}</span>
              <V1_Pill kind={pick.confidence} />
            </div>
            <div style={{ fontSize: 13, color: V1_C.textMuted, fontWeight: 500 }}>{pick.name}</div>
          </div>
          <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)' }}>
            <div style={{ fontSize: 11, color: V1_C.textSubtle, letterSpacing: '0.04em' }}>
              EP {pick.ep} · {V1A_daysAgo(pick.mention_date)}天前
            </div>
            <div style={{ fontSize: 11, color: V1_C.textMuted, marginTop: 2 }}>
              {pick.mention_date}
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          background: V1_C.surfaceAlt, color: V1_C.textMuted,
      ...
      <div style={{ padding: '18px 20px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 14 }}>
          <div style={{
            ...
            color: isPositive ? V1_C.up : V1_C.down, letterSpacing: '-0.03em',
            lineHeight: 1, fontVariantNumeric: 'tabular-nums',
          }}>
            {V1_fmt(exp.expected, 1)}
          </div>
          <div style={{ fontSize: 12, color: V1_C.textMuted, fontFamily: 'var(--font-mono)' }}>
            信心區間 {V1_fmt(exp.low, 1)} ~ {V1_fmt(exp.high, 1)}
          </div>
        </div>
        ...
        <div style={{ position: 'relative', height: 28, marginBottom: 4 }}>
          ...
          <div style={{
            ...
            width: 1, background: V1_C.borderStrong,
          }} />
          {/* bar */}
          <div style={{
            position: 'absolute',
            ...
            top: 10, height: 8,
            background: isPositive ? V1_C.upBg : V1_C.downBg,
            border: `1px solid ${isPositive ? V1_C.up : V1_C.down}`,
            borderRadius: 2,
          }} />
          {/* expected dot */}
          <div style={{
            ...
            background: isPositive ? V1_C.up : V1_C.down, borderRadius: 1,
          }} />
          {/* scale labels */}
          <div style={{
            position: 'absolute', bottom: -2, left: 0, right: 0,
            display: 'flex', justifyContent: 'space-between',
            fontSize: 9.5, color: V1_C.textSubtle, fontFamily: 'var(--font-mono)',
          }}>
            <span>{V1_fmt(-range, 0)}</span>
            <span>0</span>
        ...
        <div style={{
          ...
          gap: 8, paddingTop: 12, borderTop: `1px dashed ${V1_C.border}`,
        }}>
          {[
            ...
            <div key={s.label}>
              <div style={{ fontSize: 10, color: V1_C.textSubtle, marginBottom: 2, fontWeight: 500 }}>
                {s.label}
              </div>
              <div style={{
                fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-mono)',
                color: s.val >= 0 ? V1_C.up : V1_C.down,
                fontVariantNumeric: 'tabular-nums',
              }}>
                {V1_fmt(s.val, 1)}
              </div>
              <div style={{ fontSize: 9.5, color: V1_C.textSubtle, fontFamily: 'var(--font-mono)' }}>
                n={s.n}
              </div>
            </div>
          ))}
      ...
      <div style={{
        padding: '14px 20px',
        background: V1_C.surfaceAlt,
        borderTop: `1px solid ${V1_C.border}`,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: V1_C.textMuted, lineHeight: 1.4 }}>
            {timing.detail}
          </div>
        </div>
        <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)', flexShrink: 0 }}>
          <div style={{ fontSize: 10, color: V1_C.textSubtle, letterSpacing: '0.04em' }}>
            現價 / 當時
          </div>
          <div style={{ fontSize: 13, fontWeight: 600, color: V1_C.text, marginTop: 2 }}>
            {ccy}{V1A_fmtPrice(timing.current, market)}
          </div>
          <div style={{
            fontSize: 11, color: timing.change >= 0 ? V1_C.up : V1_C.down,
            fontVariantNumeric: 'tabular-nums',
          }}>
      ...
      <div style={{
        ...
        <div style={{
          fontSize: 12.5, color: V1_C.textMuted, lineHeight: 1.55,
...
// ─── Action 頁面主體 ────────────────────────────────
function V1A_ActionPage({ market, config }) {
  const { episodes, picks } = window.GOOAYE_DATA;
  ...
  const expectations = candidates.map(p =>
    V1A_computeExpected(p, picks, market).expected
  );
  const avgExpected = expectations.length
    ? expectations.reduce((s, v) => s + v, 0) / expectations.length
    : 0;
  const timings = candidates.map(p => V1A_entryTiming(p));
  ...
  return (
    <div style={{ padding: '28px 36px 60px', maxWidth: 1680, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{
          fontSize: 11, color: V1_C.accent, fontFamily: 'var(--font-mono)',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, flexWrap: 'wrap' }}>
          <h1 style={{
            ...
            color: V1_C.text, letterSpacing: '-0.02em', lineHeight: 1.2,
          }}>
            今天該跟哪幾檔？
          </h1>
          <div style={{ fontSize: 13, color: V1_C.textMuted }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', borderRight: `1px solid ${V1_C.border}` }}>
          <div style={{ fontSize: 10, color: V1_C.textSubtle, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, marginBottom: 6 }}>
            可跟標的
          </div>
          <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-mono)', color: V1_C.text, letterSpacing: '-0.02em', lineHeight: 1 }}>
            {candidates.length}
          </div>
          <div style={{ fontSize: 11, color: V1_C.textMuted, marginTop: 4 }}>
            來自 4 集 · 已隱藏「只是提到」
          </div>
        </div>
        <div style={{ padding: '16px 20px', borderRight: `1px solid ${V1_C.border}` }}>
          ...
          <div style={{
            ...
            color: avgExpected >= 0 ? V1_C.up : V1_C.down,
            letterSpacing: '-0.02em', lineHeight: 1,
            fontVariantNumeric: 'tabular-nums',
          }}>
            {V1_fmt(avgExpected, 1)}
          </div>
          <div style={{ fontSize: 11, color: V1_C.textMuted, marginTop: 4 }}>
            綜合三種歷史模型
          </div>
        ...
        <div style={{ padding: '16px 20px', borderRight: `1px solid ${V1_C.border}` }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-mono)', color: V1_C.up, letterSpacing: '-0.02em', lineHeight: 1 }}>
            {greatCount}<span style={{ fontSize: 16, color: V1_C.textSubtle, fontWeight: 500 }}> / {candidates.length}</span>
          </div>
          <div style={{ fontSize: 11, color: V1_C.textMuted, marginTop: 4 }}>
            當前價近當時進場價
          </div>
        ...
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ fontSize: 11, color: V1_C.textMuted, marginTop: 4, fontFamily: 'var(--font-mono)' }}>
            {V1A_daysAgo(latestEpisodes[0].date)} 天前 · {latestEpisodes[0].date}
          </div>
        </div>
      </div>
      ...
      {candidates.length === 0 ? (
        <div style={{
          background: V1_C.surface, border: `1px solid ${V1_C.border}`,
          borderRadius: 8, padding: '60px 20px', textAlign: 'center',
          color: V1_C.textMuted, fontSize: 14,
        }}>
          此市場最新 4 集沒有符合條件的可跟標的。
          <div style={{ marginTop: 8, fontSize: 12, color: V1_C.textSubtle }}>
        ...
        <div style={{
          ...
          {candidates.map(pick => {
            ...
            return (
              <V1A_ActionCard key={`${pick.ep}-${pick.ticker}`}
      ...
      <div style={{
        ...
        background: V1_C.surfaceAlt, borderRadius: 6,
        fontSize: 11, color: V1_C.textMuted, lineHeight: 1.6,
      }}>
        <strong style={{ color: V1_C.text }}>預期回報計算方式：</strong>
        綜合三個歷史來源的 1M 平均報酬 —
        (1) 相同信心度類別的所有個股、
        (2) 該個股過去被提及時的表現、
        (3) 同產業同信心度的歷史平均。
        ...
        <strong style={{ color: V1_C.text }}>本頁僅供參考，並非投資建議。</strong>
      </div>
    </div>
  );
}

Object.assign(window, { V1A_ActionPage });
